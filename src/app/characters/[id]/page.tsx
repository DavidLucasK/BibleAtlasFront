...
import styles from "./MapChar.module.css";
...
export default function Character() {
  ...
  useEffect(() => {
    ...
    async function getEventDetails() {
      try {
        const res = await fetch(
          `https://bibleatlas.vercel.app/api/event_details?event_id=${id}`
        );
        const data = await res.json();
    ...
    <div className={styles.pageContainer}>
      ...
      <div>
        {!loading && convertedPins.length > 0 && (
          <Map pins={convertedPins} center={center} details={eventDetails} />
        )}
      </div>
      <Footer />
    </div>
